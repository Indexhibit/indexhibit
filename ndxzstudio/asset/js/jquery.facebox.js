...
(function($) 
{
	$.facebox = function(data, klass) 
	{
	  ...
	  $.extend($.facebox, {
	    settings: {
	      ...
	      opacity      : 0.5,
	      overlay      : true,
	      loadingImage : 'asset/img/char.gif',
  	...
  	$.fn.facebox = function(settings) 
	{
	    ...
	    function clickHandler() 
		{
			  ...
			  // additional options added by Vaska for Indexhibit system
			  addRefSettings(this.rel);
	...
	$.facebox.resetter = function()
	{
  	...
  	function addRefSettings(ref)
	  {
			...
			{
				var refTmp = new Array();
				refTmp = ref.split(';');
			
				// need to drop the first one - it's facebox
			
				var total = refTmp.length;

				for (i=0; i<total; i++)
				{
					if (i != 0)
					{
						var optTmp = new Array();
						var tmp = refTmp[i];
						optTmp = tmp.split('=');
					
						$.facebox.settings[optTmp[0]] = optTmp[1];
					}
				}
			}
	  }
	...
	{
		if ($.facebox.settings.inited == true) 
		{	
				return true;
		}
		else 
...
// outside because we're arriving from an iframe
function faceboxClose(reload)
{
	if (reload)
	{
		$(document).trigger('close.facebox');
		$.facebox.settings.inited = false;
		$('#img-container a').unbind('click');
		jQuery('#img-container a[rel*=facebox]').facebox();
		return false;
	}
	else
	{
