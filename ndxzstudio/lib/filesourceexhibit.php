	...
	public function getExhibitImages($id=0)
	{
		...
		if ($imgs)
		{
			foreach ($imgs as $img)
			{
				...
				$active = ($img['media_hide'] == 1) ? ' file-inactive' : '';

				$cover = ($cover == true) ? "<div style='background: #000; position: absolute; z-index: 1; bottom: -23px; right: 0px; border: 1px solid #000; color: #fff; padding: 1px 2px; font-weight: bold; text-transform: uppercase; font-size: 8px;'>C</div>" : '';
				
				$add = (!in_array($img['media_mime'], $medias)) ? "<div style='position: absolute; z-index: 1; top: 0px; right: 0px; border-bottom: 1px solid #fff; border-left: 1px solid #fff; color: #fff; padding: 1px 6px; font-weight: bold; text-transform: uppercase; font-size: 8px;' class='file-$img[media_mime]'>$img[media_mime]</div>$cover" : "<div style='position: absolute; z-index: 1; top: 0px; right: 0px; border-bottom: 1px solid #fff; border-left: 1px solid #fff; color: #fff; padding: 1px 3px; font-weight: bold; text-transform: uppercase; font-size: 8px;' class='file-$img[media_mime]{$active}'>$img[media_mime]</div>$cover";
				
				$body .= "<li class='box' id='box$img[media_id]' title='$img[media_file]' style=\"position: relative; background-image: url('$thumb');\"><span class='drag-img'></span>$add<br /><a href='#' onclick=\"deleteImage($img[media_id],$img[media_ref_id],'" . rawurlencode($img['media_file']) . "'); return false;\" style='color: #999;'><img src='asset/img/img-delete.gif' title='".$OBJ->lang->word('delete')."' style='width: 11px; height: 11px;' /></a> <a href='?a=system&amp;q=img&amp;id=$img[media_id]' rel=\"facebox;height=450;width=810\" style='color: #999;'><img src='asset/img/img-edit.gif' title='".$OBJ->lang->word('edit')."' style='width: 11px; height: 11px;' /></a></li>\n\n";
			}
		}
		else
		{
